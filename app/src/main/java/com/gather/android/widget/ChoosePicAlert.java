...
public final class ChoosePicAlert {
	...
	public static Dialog showAlert(final Context context, final String title, final String[] items, String exit, final OnAlertSelectId alertDo, OnCancelListener cancelListener) {
		...
		AlertsAdapter adapter = new AlertsAdapter(context, title, items, exit, cancel);
...
class AlertsAdapter extends BaseAdapter {
	...
	public AlertsAdapter(Context context, String title, String[] items, String exit, String cancel) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null || ((ViewHolder) convertView.getTag()).type != type) {
			...
			if (type == TYPE_CANCEL) {
				convertView = View.inflate(context, R.layout.alert_dialog_choose_pic_cancel, null);
			} else if (type == TYPE_BUTTON) {
				if (position == items.size() - 2) {
					convertView = View.inflate(context, R.layout.alert_dialog_choose_pic_buttom, null);
				} else if (position == 1) {
					convertView = View.inflate(context, R.layout.alert_dialog_choose_pic_top, null);
				} else {
