...
import com.gather.android.adapter.CollectionActListAdapter;
 ...
 * 收藏的活动列表
 */
public class CollectionActListFragment extends BaseFragment {

	private View convertView;
	private XListView listView;
	private CollectionActListAdapter adapter;
	...
	protected void OnCreate(Bundle savedInstanceState) {
		...
		this.listView.setXListViewListener(new IXListViewListener() {
			...
			public void onRefresh() {
				adapter.getCollectionActList(userId);
			}
		...
		this.adapter = new CollectionActListAdapter(getActivity());
			...
			public void refreshOver(int code, String msg) {
				...
				case 0:
					if (msg.equals(SuperAdapter.ISNULL)) {
						listView.setFooterImageView("还没有收藏的活动", R.drawable.no_result);
					} else {
		...
		this.adapter.getCollectionActList(userId);
