	...
	public void loadMore() {
		if (isOver == 1) {
			...
			if (!isRequest) {
				...
				GetMessageUserListParam param = new GetMessageUserListParam(mContext, cityId, page, limit);
	...
	private void CancelUser(final MessageUserListModel model) {
		...
		dialog.setMessage("您确定要删除？").withDuration(400).withEffect(Effectstype.Fadein).setOnClick(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				mLoadingDialog.show();
				CancelMessageUserParam param = new CancelMessageUserParam(mContext, model.getId());
				HttpStringPost task = new HttpStringPost(mContext, param.getUrl(), new ResponseListener() {
					@Override
					public void success(int code, String context, String result) {
