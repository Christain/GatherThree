...
import com.gather.android.adapter.ActHasSignListAdapter;
 ...
 * 活动已签到列表
 */
public class ActHasSignListFragment extends BaseFragment {

	private View convertView;
	private XListView listView;
	private ActHasSignListAdapter adapter;
	...
	protected void OnCreate(Bundle savedInstanceState) {
		...
		this.listView.setXListViewListener(new IXListViewListener() {
			...
			public void onRefresh() {
				adapter.getActHasSignList(userId);
			}
		...
		this.adapter = new ActHasSignListAdapter(getActivity());
			...
			public void refreshOver(int code, String msg) {
				...
				case 0:
					if (msg.equals(SuperAdapter.ISNULL)) {
						listView.setFooterImageView("还没有已签到的活动", R.drawable.no_result);
					} else {
		...
		this.adapter.getActHasSignList(userId);
	}
