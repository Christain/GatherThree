...
import com.gather.android.adapter.ActHasCommentListAdapter;
 ...
 * 活动已评论列表
 */
public class ActHasCommentListFragment extends BaseFragment {

	private View convertView;
	private XListView listView;
	private ActHasCommentListAdapter adapter;
	...
	protected void OnCreate(Bundle savedInstanceState) {
		...
		this.listView.setXListViewListener(new IXListViewListener() {
			...
			public void onRefresh() {
				adapter.getActHasCommentList(userId);
			}
		...
		this.adapter = new ActHasCommentListAdapter(getActivity());
			...
			public void refreshOver(int code, String msg) {
				...
				case 0:
					if (msg.equals(SuperAdapter.ISNULL)) {
						listView.setFooterImageView("还没有已评论的活动", R.drawable.no_result);
					} else {
		...
		this.adapter.getActHasCommentList(userId);
